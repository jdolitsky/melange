...
func (l *lima) Run(cfg *Config, args ...string) error {
	...
	baseargs := []string{"exec", "-u", "build", "-w", "/home/build"}
...
func (l *lima) StartPod(cfg *Config) error {
	...
	args = append(args, cfg.ImgDigest)
...
func (l limaOCILoader) LoadImage(layerTarGZ string, arch apko_types.Architecture, bc *apko_build.Context) (dig name.Digest, err error) {
	...
	if err := apko_oci.BuildImageTarballFromLayer(
		...
		return dig, fmt.Errorf("failed to build OCI image: %w", err)
	}
	f, err := os.Open(outputTarGZ)
	if err != nil {
		return dig, fmt.Errorf("failed to open OCI image: %w", err)
	}
	defer f.Close()
	...
	if err := l.lima.nerdctl(melangeVMName, f, &buf, &errBuf, "image", "load", fmt.Sprintf("--platform=%s", arch)); err != nil {
		return dig, fmt.Errorf("failed to load image into containerd: %w %s", err, errBuf.Bytes())
	}
	...
	if len(matches) != 3 {
		return dig, fmt.Errorf("failed to find digest for loaded image: %s", out)
	}
	return name.NewDigest(fmt.Sprintf("%s@%s", matches[1], matches[2]))
}
